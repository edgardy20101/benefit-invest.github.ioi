...
class QIWI {
    ...
    public function GetHistory( $sStartDate, $sFinishDate ) {
        ...
        foreach( explode( '</div><div class="reportsLine ', str_replace( '> <', '><', preg_replace( '!\s+!u', ' ', $sResult ) ) ) as $iKey => $sValue ) {
            ...
            $aData['sCurrency'] = strpos( $aData['sAmount'], 'руб.' ) !== false ? 'RUB' : (strpos( $aData['sAmount'], 'долл.' ) !== false ? 'USD' : (strpos( $aData['sAmount'], 'тенге.' ) !== false ? 'KZT' : 'NAN'));
            ...
            # Прибыль или расход ?
            $aData['sType'] = strpos( $sValue, 'IncomeWithExpend expenditure' ) !== false ? 'EXPENDITURE' : (strpos( $sValue, 'IncomeWithExpend income' ) !== false ? 'INCOME' : 'NAN');
    ...
    public function GetInvoices( $sStartDate, $sFinishDate ) {
        ...
        foreach( explode( '<div class="ordersLine ', str_replace( '> <', '><', preg_replace( '!\s+!u', ' ', $sResult ) ) ) as $iKey => $sValue ) {
            ...
            # Получение валюты счета :
            $aData['sCurrency'] = strpos( $aData['sAmount'], 'евро.' ) !== false ? 'EUR' : (strpos( $aData['sAmount'], 'руб.' ) !== false ? 'RUB' : (strpos( $aData['sAmount'], 'долл.' ) !== false ? 'USD' : (strpos( $aData['sAmount'], 'тенге.' ) !== false ? 'KZT' : 'NAN')));
    ...
    public function payment( $iProvider, array $aExtra, $dAmount, $sCurrency = 'RUB', $sPayCurrency = 'RUB' ) {
        ...
        if( strpos( $sResponse, 'transaction":"' ) === false ) {
            if( strpos( $sResponse, 'class="errorElement"' ) !== false ) {
            ...
            else if( strpos( $sResponse, 'confirmPage' ) !== false )
                return false;
            else {
